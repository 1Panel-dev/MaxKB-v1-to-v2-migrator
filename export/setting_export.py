...
def log_export(log_list, current_page):
    batch_list = [LogModel(log).data for log in log_list]
    save_batch_file(batch_list, 'log', current_page)


def model_export(model_list, current_page):
    batch_list = [ModelModel(model).data for model in model_list]
    save_batch_file(batch_list, 'model', current_page)


def system_setting_export(system_setting_list, current_page):
    batch_list = [SystemSettingModel(system_setting).data for system_setting in system_setting_list]
    save_batch_file(batch_list, 'system_setting', current_page)


def team_member_permission_export(team_member_permission_list, current_page):
    ...
    save_batch_file(batch_list, 'team_member_permission', current_page)


def user_export(user_list, current_page):
    batch_list = [UserModel(user).data for user in user_list]
    save_batch_file(batch_list, 'user', current_page)


def export():
    page(QuerySet(Log), 100, log_export, "导出操作日志")
    page(QuerySet(Model), 100, model_export, "导出模型")
    page(QuerySet(SystemSetting), 100, system_setting_export, "导出系统设置", primary_key="type")
    page(QuerySet(TeamMemberPermission), 100, team_member_permission_export, "导出团队授权数据")
    page(QuerySet(User), 100, user_export, "导出用户数据")
