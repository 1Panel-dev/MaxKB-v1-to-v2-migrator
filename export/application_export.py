...
def application_workflow_version_export(workflow_version_list, current_page):
    ...
    save_batch_file(batch_list, "application_version", current_page)


def application_api_key_export(application_api_key_list, current_page):
    batch_list = [ApplicationApiKeyModel(application_api_key).data for application_api_key in application_api_key_list]
    save_batch_file(batch_list, "application_api_key", current_page)


def application_access_token_export(application_access_token_list, current_page):
    ...
    save_batch_file(batch_list, "application_access_token", current_page)


def application_public_access_client_export(application_public_access_client_list, current_page):
    ...
    save_batch_file(batch_list, 'application_public_access_client', current_page)


def chat_export(chat_list, current_page):
...
def export():
    page(QuerySet(Application), 50, application_export, "导出应用")
    page(QuerySet(WorkFlowVersion), 50, application_workflow_version_export, "导出应用工作流历史版本")
    page(QuerySet(ApplicationApiKey), 50, application_api_key_export, "导出应用Apikey")
    page(QuerySet(ApplicationAccessToken), 50, application_access_token_export, "导出应用访问限制配置",
